...
const SYSTEM_PROMPT = `You are a UI/UX expert analyzing screenshots from e2e tests. Your job is to carefully examine the image and identify any visual issues, bugs, or problems.
...
async function analyzeWithOpenCode(imagePath: string): Promise<string> {
...
function parseOutput(output: string): AnalysisResult {
  ...
  // The output may contain ANSI codes and other text, so we need to find the JSON part
  ...
  if (!jsonMatch) {
    ...
    // Check if output contains "NO ISSUES" case-insensitively
    ...
    // Check if output indicates issues were found
    const hasIssuesIndicator =
      output.toLowerCase().includes("issue") ||
      output.toLowerCase().includes("bug") ||
      output.toLowerCase().includes("problem") ||
      output.toLowerCase().includes("wrong") ||
      output.toLowerCase().includes("incorrect");

        ...
        function* (sessionID: SessionID) {
          ...
          while (true) {
            ...
            const outcome: "break" | "continue" = yield* Effect.gen(function* () {
              ...
              const lastUserText = lastUserMsg ? lastUserMsg.parts.filter((p) => p.type === "text" && !p.ignored && !p.synthetic).map((p) => ("text" in p ? p.text : "")).join("\n") : ""

              const [skills, env, instructions, modelMsgs, projectMemory] = yield* Effect.all([
                sys.skills(agent),
                Effect.sync(() => sys.environment(model)),
                instruction.system().pipe(Effect.orDie),
                MessageV2.toModelMessagesEffect(msgs, model),
                memory.query({ projectID: Instance.project.id, query: lastUserText, limit: 5 }),
              ])
              ...
              const memorySection =
                projectMemory.length > 0
                  ? [
                      "",
                      "## Project Memory",
                      "The following information was learned from previous sessions and may be relevant:",
                      ...projectMemory.map((m) => `- ${m.content}`),
                    ].join("\n")
                  : ""

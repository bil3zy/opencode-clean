...
const parse = Effect.fn("BashTool.parse")(function* (command: string, ps: boolean) {
  // @ts-expect-error parser is lazy but internally returns Promise
  const tree = (yield* Effect.promise(() => parser().then((p) => (ps ? p.ps : p.bash).parse(command)))) as { rootNode: Node }
